...
"STLink programmer/debugging utility $Id: stlink-download.c 18 2011-06-22 14:26:36Z donald.becker@gmail.com $  Copyright Donald Becker";
...
#else
struct stlink *stl_init(struct stlink *sl, const char *dev_name, HANDLE fd)
#endif
{
...
SCSI_PASS_THROUGH_DIRECT_WITH_BUFFER sptdwb;
  ...
  int i;
	...
	if (stl->verbose) {
		fprintf(stderr, "\nCDB: ");
		for(i=0;i<16;i++)
			fprintf(stderr, "%2.2X ",sptdwb.sptd.Cdb[i]);
		fprintf(stderr, "\n");
	}
	...
	if (stl->verbose) {
		...
		fprintf(stderr, "Data: ");
		for(i=0;i<20;i++)
			fprintf(stderr, "%2.2X ",stl->q_buf[i]);
		fprintf(stderr, "\n");
	}
...
#else
int stl_read(struct stlink* sl, stm32_addr_t addr, void *buf, size_t size)
#endif
{
	size_t offset = 0;
...
#else
	        sl->fd = CreateFile("\\\\.\\E:",         // drive to open
...
static void stm_discovery_blink(struct stlink* sl)
{
	...
	if (sl->verbose)
		fprintf(stderr, "GPIOC_CRH = 0x%08x\n", PortC_hi_iocfg);
...
int main(int argc, char *argv[])
{
	...
	struct stlink *sl;
        ...
        program = argv[0];
        if((argc > 2) || (argc == 2) && (strstr(argv[1],"?")||strstr(argv[1],"help")))
		fprintf(stderr, usage_msg, program, program);
#ifndef WINDOWS
	dev_name = "/dev/stlink";
#else	
	dev_name = "\\\\.\\E:";
...
#else
	fd = CreateFile(dev_name,         // drive to open
        	GENERIC_READ | GENERIC_WRITE,
                FILE_SHARE_READ | FILE_SHARE_WRITE,
                NULL,             // default security attributes
                OPEN_EXISTING,    // disposition
